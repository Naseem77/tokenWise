...
class ContextSelector:
    ...
    def _select_with_dependencies(
        ...
        for sc in scored_chunks:
            ...
            cluster_tokens = sum(
                c.chunk.token_count or count_tokens(c.chunk.text)
                for c in cluster
            )
            ...
            if current_tokens + cluster_tokens <= token_budget:
                for chunk in cluster:
                    if chunk.chunk.id not in selected_ids:
                        ...
                        current_tokens += (
                            chunk.chunk.token_count or 
                            count_tokens(chunk.chunk.text)
                        )
            else:
    ...
    def reorder_chunks(
        ...
        for source in sorted(sources.keys(), 
                           key=lambda s: max(sc.relevance_score for sc in sources[s]),
                           reverse=True):
