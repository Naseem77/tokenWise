...
class RelevanceRanker:
    ...
    async def rank_chunks(
        ...
        for i, chunk in enumerate(chunks):
            ...
            if use_embedding and query_embedding and chunk_embeddings:
                embedding_score = calculate_cosine_similarity(
                    query_embedding, 
                    chunk_embeddings[i]
                )
    ...
    def _calculate_relationship_score(
        ...
        valid_relationships = sum(
            1 for rel_id in chunk.relationships 
            if rel_id in chunk_ids
        )
        ...
        for sc in scored_chunks:
            if sc.chunk.id not in high_scoring_ids:
                ...
                for high_id in high_scoring_ids:
                    if (high_id in relationships.get(sc.chunk.id, set()) or
                        sc.chunk.id in relationships.get(high_id, set())):
                        # Boost this chunk
